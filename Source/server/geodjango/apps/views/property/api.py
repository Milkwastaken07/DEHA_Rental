...
#         keys = ["favoriteIds", "priceMin", "priceMax", "beds", "baths", "propertyType", "squareFeetMin", "squareFeetMax", "amenities", "availableFrom", "latitude", "longitude"]
#         favoriteIds, priceMin, priceMax, beds, baths, propertyType, squareFeetMin, squareFeetMax, amenities, availableFrom, latitude, longitude = [params.get(key) for key in keys]
#         filters = {}

#         if favoriteIds:
#             try:
#                 favoriteIdsArray = [int(id) for id in favoriteIds.split(',') if id and id.isdigit()]
#                 if favoriteIdsArray:
#                     filters['id__in'] = favoriteIdsArray
...
#         if priceMin:
#             try:
#                 filters['pricePerMonth__gte'] = float(priceMin)
...
#         if priceMax:
#             try:
#                 filters['pricePerMonth__lte'] = float(priceMax)
#             except ValueError as e:
#                 logging.error(f"Errors: {e}")
#                 pass
...
#             try:
#                 filters['baths__gte'] = float(baths)
#             except ValueError as e:
#                 logging.error(f"Errors: {e}")
#                 pass
...
#         if squareFeetMin:
#             try:
#                 filters['squareFeet__gte'] = int(squareFeetMin)
#             except ValueError as e:
#                 logging.error(f"Errors: {e}")
#                 pass
...
#         if squareFeetMax:
#             try:
#                 filters['squareFeet__lte'] = int(squareFeetMax)
#             except ValueError as e:
#                 logging.error(f"Errors: {e}")
#                 pass

#         # Xử lý propertyType
#         if propertyType is not None and propertyType != "any":
#             valid_types = [choice[0] for choice in Property.PROPERTY_TYPE_CHOICES]
#             if propertyType in valid_types:
...
#             if amenitiesArray:
#                 filters['amenities__contains'] = amenitiesArray

#         # Xử lý availableFrom
#         if availableFrom is not None:
#             try:
#                 date = parser.parse(availableFrom)
#                 subquery = Lease.objects.filter(
#                     property_id=OuterRef('pk'),
#                     startDate__lte=date
#                 )
#                 filters['pk__in'] = Property.objects.filter(Exists(subquery)).values('pk')
#             except ValueError as e:
#                 logging.error(f"Errors: {e}")
#                 pass
...
#                 lat = float(latitude)
#                 lng = float(longitude)
#                 radius_in_kilometers = 1000
#                 point = Point(lng, lat, srid=4326)
#                 # Lọc Property dựa trên Location.coordinates
#                 subquery = Property.objects.filter(
#                     location__coordinates__dwithin=(point, D(km=radius_in_kilometers))
#                 )
#                 filters['pk__in'] = subquery.values('pk')
#             except ValueError as e:
#                 logging.error(f"Errors: {e}")
#                 pass

#         # Lọc dữ liệu với join đến Location
#         queryset = Property.objects.select_related('locationId').filter(**filters)
        ...
        if latitude and longitude:
            try:
                ...
                queryset = queryset.filter(
                    location__coordinates__dwithin=(reference_point, Distance(km=radius_in_kilometers))
                )
            except ValueError:
                pass
