...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            # Reset sequences cho tất cả các bảng sau khi insert
            for _, model in model_files:
                table_name = model._meta.db_table
                self.reset_sequence(table_name)
                self.stdout.write(f'Reset sequence for table: {table_name}')

            self.stdout.write(self.style.SUCCESS('Successfully seeded database and reset sequences'))
