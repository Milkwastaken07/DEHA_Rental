...
import { Application, Lease, Manager, Property, Tenant } from "@/types/models";
    ...
    getAuthUser: build.query<User, void>({
      queryFn: async (_, _queryApi, _extraoptions, fetchWithBQ) => {
        try {
          ...
          if (userDetailsRespone.error) {
            console.error("Error details:", userDetailsRespone.error);
            if (userDetailsRespone.error.status === 404) {
              ...
              return { error: userDetailsRespone.error };
            }
          }
          return {
            data: {
              cognitoInfo: { ...user },
              userInfo: userDetailsRespone.data as Tenant | Manager,
              userRole,
            },
          };
        } catch (error: any) {
