...
def getTenant(cognitoId: str):
    try:
        tenant = Tenant.objects.get(cognitoId = cognitoId)
        logging.info(f"Found Tenant: {tenant} (cognitoId: {cognitoId})")
        return tenant
...
def updateTenant(cognitoId: str, name: str, email: str, phoneNumber: str):
    ...
    try:
        # Validate inputs
        if not all([cognitoId, name, email, phoneNumber]):
            raise ValueError("All fields (cognitoId, name, email, phoneNumber) are required")
        updateTenant = Tenant.objects.get(cognitoId=cognitoId)
        updateTenant.name = name  # Remove the comma
        updateTenant.email = email  # Remove the comma
